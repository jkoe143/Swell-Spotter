...
## Ports Dataset (for port plotting and search)

- Place the dataset file at `frontend/public/database.json`.
- Expected format is an array of entries like:
...
- At runtime, the app tries to load `/database.json` (or `/ports.json` as a fallback). If neither is present, it attempts to load the public dataset from GitHub.
- After adding the file, restart the dev server to ensure it’s served.
